...
<section>
  ...
  <p class="mt-1 text-sm text-secondary">
    ...
    its <a class="text-accent hover:underline" href={resolve('/schemas/tierdom-import-v1.json')}
      >JSON Schema</a
    > is published so you can wrangle data from any system into it.
  </p>

  <ul class="mt-6 grid gap-3 sm:grid-cols-2">
    {#each data.importers as importer (importer.id)}
      {@const Icon = iconFor(importer)}
      {@const available = importer.status === 'available'}
      <li>
        <a
          href={resolve(`/admin/tools/import/${importer.id}`)}
          class="flex h-full items-start gap-4 rounded-lg border border-subtle bg-elevated px-5 py-4 transition-colors hover:border-accent/40"
  ...
  <a
    ...
    class="mt-3 flex items-start gap-4 rounded-lg border border-dashed border-subtle bg-canvas px-5 py-4 transition-colors hover:border-accent/40"
  >
    ...
    <div class="min-w-0">
      ...
      <p class="mt-1 text-xs text-secondary">
        Want a built-in importer for another system? Open or upvote an issue on the project site.
      </p>
    </div>
  </a>
</section>
